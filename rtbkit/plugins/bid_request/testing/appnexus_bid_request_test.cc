...
BOOST_AUTO_TEST_CASE( test_openrtb_from_appnexus )
{
    ...
    // 96.246.152.18
    // OpenRTB::Device
    BOOST_CHECK_EQUAL(ortbReq->device->ua, std::string("Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.5; en-US;rv:1.9.0.3) Gecko/2008092414 Firefox/3.0.3"));
    BOOST_CHECK_EQUAL(ortbReq->device->language, std::string("en-US,en;q=0.8"));
    ...
    BOOST_CHECK_EQUAL(ortbReq->device->carrier, std::string("101"));
    BOOST_CHECK_EQUAL(ortbReq->device->language, std::string("en-US,en;q=0.8"));
    BOOST_CHECK_EQUAL(ortbReq->device->make, std::string("1001"));
    BOOST_CHECK_EQUAL(ortbReq->device->model, std::string("10001"));
    BOOST_CHECK_EQUAL(ortbReq->device->geo->country, "US");
    BOOST_CHECK_EQUAL(ortbReq->device->geo->region, "NY");
    BOOST_CHECK_EQUAL(ortbReq->device->geo->city, std::string("New York"));
    BOOST_CHECK_EQUAL(ortbReq->device->geo->zip, std::string("10014"));
    ...
    BOOST_CHECK_EQUAL(ortbReq->device->os, std::string("iPhone"));
    BOOST_CHECK_EQUAL(ortbReq->device->osv, std::string("N/A"));
