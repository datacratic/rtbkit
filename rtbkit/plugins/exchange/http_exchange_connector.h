    ...
    virtual std::function<double(double)> getLoadSampleFn() const
    {
        ...
        return [=] (double elapsed) mutable -> double {
            ...
            for (auto i = 0; i < sample.size(); ++i) {
                auto s = double(sample[i].ru_utime.tv_sec - lastSample[i].ru_utime.tv_sec)
                       + double(sample[i].ru_stime.tv_sec - lastSample[i].ru_stime.tv_sec);
                auto u = double(sample[i].ru_utime.tv_usec - lastSample[i].ru_utime.tv_usec)
                       + double(sample[i].ru_stime.tv_usec - lastSample[i].ru_stime.tv_usec);
            ...
            lastSample = std::move(sample);
            std::cerr << "THREAD LOAD :" << maximum << std::endl;
            return maximum;
        };
    }
