...
struct Init {
    Init() {
        ...
        PluginInterface<BidRequest>::registerPlugin("spotx_2.1", [](const std::string& request) {
            return OpenRTBBidRequestParser::openRTBBidRequestParserFactory("2.1")->parseBidRequest(request,
                SpotXExchangeConnector::exchangeNameString(), SpotXExchangeConnector::exchangeNameString());
        });

        PluginInterface<BidRequest>::registerPlugin("spotx_2.2", [](const std::string& request) {
